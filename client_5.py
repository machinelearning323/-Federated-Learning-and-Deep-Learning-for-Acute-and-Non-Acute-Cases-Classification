...
class FlowerClient(fl.client.NumPyClient):
    ...
    def save_metrics(self):
        ...
        np.save("client5_precision.npy", np.array(self.precision_history))
        np.save("client5_recall.npy", np.array(self.recall_history))
        np.save("client5_accuracy.npy", np.array(self.accuracy_history))

        print(f"Metrics saved for client 5")
...
if __name__ == "__main__":
    ...
    client_id = 5  # Change this for each client
    train_data, val_data, train_labels, val_labels = train_test_split(partitions[client_id][0], partitions[client_id][1], test_size=0.2)
